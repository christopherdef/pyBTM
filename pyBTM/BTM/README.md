...
The code includes a runnable example that takes in three command line arguments: input file, output directory and # of iterations, you can run it *on Linux* by:

       $ cd script
	   $ sh runExample.sh input.txt output_dir iteration

The # of topics and be modified inside the script. It is also modified to save intermediate steps as models, which can be modified in the script as well.  
