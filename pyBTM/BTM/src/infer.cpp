...
void Infer::run(string docs_pt, string model_dir) {
  ...
  string pt = model_dir + "k" + str_util::itos(K) + "_n" + str_util::itos(n_iter) + "_a" + str_util::ftos(alpha) + "_b" + str_util::ftos(beta) + ".pz_d";
...
void Infer::load_para(string model_dir) {
  string pt = model_dir + "k" + str_util::itos(K) + "_n" + str_util::itos(n_iter) + "_a" + str_util::ftos(alpha) + "_b" + str_util::ftos(beta) + ".pz";
  cout << "load p(z):" << pt <<endl;
  pz.loadFile(pt);
  assert(abs(pz.sum() - 1) < 1e-4);
 
  string pt2 = model_dir + "k" + str_util::itos(K) + "_n" + str_util::itos(n_iter) + "_a" + str_util::ftos(alpha) + "_b" + str_util::ftos(beta) + ".pw_z";
