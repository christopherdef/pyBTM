...
void Model::save_res(string dir, int inter) {

  cout<< dir<< endl;
    
  string pt;
  pt = dir + "_n" + str_util::itos(n_iter) + "_a" + str_util::ftos(alpha) + "_b" + str_util::ftos(beta) +".pz";
  cout << "\nwrite p(z): " << pt << endl;
  save_pz(pt);

  string pt2;
  pt2 = dir + "_n" + str_util::itos(n_iter) + "_a" + str_util::ftos(alpha) + "_b" + str_util::ftos(beta) +".pw_z";
